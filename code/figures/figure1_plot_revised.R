...
colnames(xv_corr_df) <- c('real_xv', 'random_xv')
xv_corr_df <- melt(xv_corr_df)


svg(file = "figures/figure1/C_correlation_consensus_revised.svg", height = 5, width = 6)

ggplot(data=corr_df, aes(x=value,  fill=variable)) +
  geom_density(data=xv_corr_df, inherit.aes=F, aes(x=value, fill=variable), alpha=0.1, linetype=2, size=0.2) +
  geom_density(alpha=0.5) +
  labs(x = "Correlation", y = "Frequency") + 
  theme_classic() + 
  scale_fill_manual(values = c("#E31A1C", "#E31A1C", 'grey60', 'grey60'),
                    name = "Data",
                    labels = c("consensus", 'consensus', "random", 'random')) +
  theme(
...
# SUPPLEMENARY FIGURE 1 - THIS IS NOW FIGURE 2
# S1A ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
biswas <- read.table("data/figures/figure1/suppl_biswas.txt", header=T)
biswas_m <- melt(biswas)

svg(file = "figures/figure1/SUPPL_biswas_revised.svg", height = 4, width = 5)

ggplot(biswas_m) + 
  geom_boxplot(aes(x=variable, y=value), fill="red") + 
...
svg(file = "figures/figure1/SUPPL_datasets_revised.svg", height = 4, width = 5)
...
svg(file = "figures/figure1/SUPPL_lecanda_revised.svg", height = 2.5, width = 4)
