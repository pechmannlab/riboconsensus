...
colnames(redundancy) <- c("length", "min 5", "min 10", "min 20")
red_m <- melt(redundancy, id='length')

redundancy_mm <- read.table("data/figures/figure3/kmer_redundancy2.txt", header=T)
colnames(redundancy_mm) <- c("length", "min 5", "min 10", "min 20")
red2_m <- melt(redundancy_mm, id='length')



svg(file = "figures/figure3/SUPPL_redundancy_revised.svg", height = 4.5, width = 4)


plot_with_mm <- ggplot(red2_m, aes(x=length, y=value+1, fill=variable)) + 
  geom_col( position="dodge2", show.legend = F ) + 
  scale_y_log10() + 
  scale_fill_manual(values=c("#000074", "#0000C8", "#0074E4")) + 
  labs(x="Length", y="Counts", title="Including MM") + 
  theme_classic() + 
  theme(
...
plot_wo_mm <- ggplot(red_m, aes(x=length, y=value+1, fill=variable)) + 
  geom_col( position="dodge2" ) + 
  scale_y_log10() + 
  scale_fill_manual(values=c("#000074", "#0000C8", "#0074E4")) + 
  labs(x="Length", y="Counts", title="Excluding MM") + 
  theme_classic()  +
  theme(
    text = element_text(size=20),
    legend.title = element_blank(), 
...
plot_grid(plot_with_mm, plot_wo_mm, labels ="", ncol = 1, align = 'v')
...
svg(file = "figures/figure3/C_counts.svg", height = 5, width = 8)
...
svg(file = "figures/figure3/D_summary.svg", height = 4, width = 4)
...
svg(file = "figures/figure3/E_codons.svg", height = 3, width = 16)
