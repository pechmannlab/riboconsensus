...
svg(file = "figures/figure2/SUPPL_syncodons_revised.svg", height = 6, width = 8)

grid <- data.frame(pos=c(1:20))
ggplot(syncodons_m) + 
  geom_point(aes(x=codon, y=variable, color=factor(value) ), size=3) + 
  geom_hline(data=grid, aes(yintercept = pos),  size=0.5) +
...
coverage <- read.table("data/figures/figure2/coverage_revised.txt", header=T)
coverage$auc <- result.inddatasets

svg(file = "figures/figure2/SUPPL_coverage_revised.svg", height = 4, width = 4)

ggplot(coverage) + 
  geom_point(aes(x=auc, y=coverage, color=auc, size=auc), show.legend = F) + 
  scale_color_viridis_c(option="A", begin=0, end=0.7, direction=1) + 
  labs(x="AUC", y="Average coverage") + 
  theme_classic() + 
  theme(
    text = element_text(size=20),
...
svg(file = "figures/figure2/D_codonpairs_histo.svg", height = 5, width = 6)
...
svg(file = "figures/figure2/E_codonpairs_rank.svg", height = 5, width = 4)
...
svg(file = "figures/figure2/F_RD_normdist.svg", height = 5, width = 5)
